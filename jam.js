...
function drawClock() {
    drawFace(ctx, radius); drawNumbers(ctx, radius);
    drawTime(ctx, radius);
}
...
function drawFace(ctx, radius) {
    ...
    grad.addColorStop(0, '#216');
    grad.addColorStop(0.5, 'skyblue');
    grad.addColorStop(1, '#919');
    ctx.strokeStyle = grad;
    ctx.lineWidth = radius * 0.1; ctx.stroke(); ctx.beginPath();
...
function drawNumbers(ctx, radius) {
    var ang; var num;
    ctx.font = radius*0.17 + "px arial"; ctx.textBaseLine = "middle";
